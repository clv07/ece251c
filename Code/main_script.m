...
alpha = 0.5;%-(0.1957 - 1.048*((2/pi)*atan(0.07212*(fs/1000))).^(1/2));
...
%Turn Channel Data into .wav
% for m = 1:size(X,1)
%     audio = X(m,:).';
%     filename = sprintf('Channel_%d.wav', m);
%     audiowrite(filename, audio, fs);
% end

%Turn Distorted .wav into New Channel Data
X_RNN = zeros(size(X));
for m = 1:size(X,1)
    file_name = sprintf('Channel_%d_RNN.wav', m);
    [RNN_data, fs_RNN] = audioread(file_name);
    X_RNN(m,:) = RNN_data;
end
[x_rec] = warped_cmfb_synthesis(WarpedCMFB, X_RNN);
%[x_rec] = warped_cmfb_synthesis(WarpedCMFB, X);

audiowrite('Example2_RNN_Reconstructed.wav',x_rec,fs);
...
% ---- Time Domain ----
subplot(2,1,1);
plot(n-170, x_rec, 'LineWidth', 1.2); hold on;
...
ylabel('Amplitude');
legend('Original x[n]', 'Reconstructed x\_rec[n-170]', 'Location', 'best');

% ---- Frequency Domain ----
subplot(2,1,2);
plot(n, 20*log10(abs(fft(x))), 'LineWidth', 1.2); hold on;
plot(n, 20*log10(abs(fft(x_rec))), 'LineWidth', 1.2); 
xlim([min(n) max(n)/2]);
grid on;
...
fig_name = 'Example2_RNN_Reconstructed_Spectrum.png';
saveas(gcf,fig_name)

figure;
% --- Row 1: Row 1 Time + Spectrum ---
subplot(3,2,1);
plot(n, X(1,:));
xlim([min(n) max(n)]);
grid on;
...
subplot(3,2,2);
plot((n/Npt)*fs, 20*log10(abs(fft(X(1,:)))), 'LineWidth', 1.5);
xlim([0 fs/2]);
grid on;
title('Channel 1 – Magnitude Spectrum');
xlabel('Frequency (Hz)');
ylabel('Magnitude (dB)');

% --- Row 2: Row 3 Time + Spectrum ---
subplot(3,2,3);
plot(n, X(3,:));
xlim([min(n) max(n)]);
grid on;
title('Channel 3 – Time Domain');
xlabel('Sample Index n');
ylabel('Amplitude');

subplot(3,2,4);
plot((n/Npt)*fs, 20*log10(abs(fft(X(3,:)))), 'LineWidth', 1.5);
xlim([0 fs/2]);
grid on;
title('Channel 3 – Magnitude Spectrum');
xlabel('Frequency (Hz)');
ylabel('Magnitude (dB)');

% --- Row 3: Row 8 Time + Spectrum ---
subplot(3,2,5);
plot(n, X(8,:));
xlim([min(n) max(n)]);
grid on;
title('Channel 8 – Time Domain');
xlabel('Sample Index n');
ylabel('Amplitude');

subplot(3,2,6);
plot((n/Npt)*fs, 20*log10(abs(fft(X(8,:)))), 'LineWidth', 1.5);
xlim([0 fs/2]);
grid on;
title('Channel 8 – Magnitude Spectrum');
xlabel('Frequency (Hz)');
ylabel('Magnitude (dB)');
